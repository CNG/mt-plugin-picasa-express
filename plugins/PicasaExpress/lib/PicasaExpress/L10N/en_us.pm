package PicasaExpress::L10N::en_us;

use strict;
use base 'PicasaExpress::L10N';
use vars qw( %Lexicon );

%Lexicon = (
  '_CONFIG_DESCRIPTION' => q{Easily insert photos from Picasa Web Albums into blog entries.},
  ...
  '_REMEMBER_TO_AUTHENTICATE' => q{You must authenticate by clicking the above button in order to use the plugin. You can save this settings form first if you have made changes. Just remember to come back!},
  '_STRUCTURE_EXPLANATION' => q{Each output image is contained within a DIV tag to allow for CSS styling or script targeting. If captions are turned on, they are output within P tags after the image tag and within the DIV tag. Here is what the output structure should look like based on last saved settings:},
  '_GOOGLE_THUMB_NOTE' => q{NOTE: Google apparently does not support dimensions larger than 2560. If cropping is enabled, the maximum size will be 2560x2560. If cropping is disabled and the width and height are blank, the original image will be used, regardless of its dimensions. Further, Google apparently supports a maximum height of 1060 pixels if no width is specified.},
